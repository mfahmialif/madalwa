    ...
    <div class="row">
        <div class="col-sm-12">
            <ul class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Data Siswa Kamar</li>
            </ul>
        </div>
    </div>
</div>
        ...
        <div class="alert alert-info d-flex align-items-center gap-2 mb-4" role="alert">
            ...
            <div>
                ...
                <small class="text-muted">
                    ...
                    Siswa hanya bisa memiliki 1 kamar per kelas untuk tracking history
                </small>
            </div>
        </div>
                    ...
                    <div class="row align-items-center">
                        <div class="col">
                            <div class="doctor-table-blk">
                                <h3><i class="fas fa-users me-2"></i>Data Siswa Kamar</h3>
                                <div class="doctor-search-blk mt-3 mt-md-0">
                                    ...
                                    <div class="add-group">
                                        <a href="{{ route('admin.kamar.siswa.add', $kamar) }}" class="btn btn-primary add-pluss ms-2">
                                            <img src="{{ asset('template') }}/assets/img/icons/plus.svg" alt="">
                                        </a>
                                        ...
                                        <a href="javascript:void(0);" onclick="deleteSiswa()" class="btn btn-primary ms-2">
                                            <img src="{{ asset('template') }}/assets/img/icons/trash.svg" alt="">
                                        </a>
                                    </div>
                                </div>
                            </div>
...
<script>
    ...
    function dataTable(tableId) {
        var url = "{{ route('admin.kamar.siswa.data', $kamar) }}"
    ...
    function deleteData(event) {
        ...
        swal({
            ...
            , text: "Siswa " + nama + " akan dihapus dari kamar ini"
            , icon: "warning"
            , buttons: {
                confirm: {
                    text: "OK"
        ...
        , }).then((willDelete) => {
            if (willDelete) {
                var url = "{{ route('admin.kamar.siswa.destroy', ['kamar' => $kamar, 'kamarSiswa' => '_kamarSiswa']) }}";
                url = url.replace('_kamarSiswa', id);
    ...
    function deleteSiswa() {
        let kamar_siswa_id = Array.from(selectedIds);

        if (kamar_siswa_id.length === 0) {
        ...
        swal({
            title: "Apa kamu yakin?"
            , text: "Siswa yang dipilih akan dihapus dari kamar ini"
            , icon: "warning"
            , buttons: {
                confirm: {
                    text: "OK"
        ...
        , }).then((willDelete) => {
            if (willDelete) {
                $.ajax({
                    type: "DELETE"
                    , url: "{{ route('admin.kamar.siswa.bulk-destroy', $kamar) }}"
                    , data: {
                        _token: "{{ csrf_token() }}"
                        , kamar_siswa_id: kamar_siswa_id
                    , }
                    , success: function(response) {
                        showToastr(response.status, response.message);
                        selectedIds.clear();
